...
const VacationSchedules = () => {
  ...
  const [vacationScheduleId, setVacationScheduleId] = useState(null);
  ...
  const handleUpdate = async (e, vacationScheduleId) => {
    e.preventDefault();
    ...
    if (token) {
      try {
        await axios.put(
          `${BASE_URL}/commander/vacationSchedule/${vacationScheduleId}`,
          editFormData,
          {
            headers: {
              token: `Bearer ${token}`,
        ...
        handleNotify(
          "success",
          "Thành công!",
          "Chỉnh sửa học viên đi tranh thủ thành công"
        );
  ...
  const handleAddFormData = async (e) => {
    ...
    const token = localStorage.getItem("token");
    try {
      await axios.post(`${BASE_URL}/commander/vacationSchedule`, addFormData, {
        headers: {
          token: `Bearer ${token}`,
        },
      });
      handleNotify(
        "success",
        "Thành công!",
        "Thêm học viên đi tranh thủ thành công"
      );
  ...
  const handleDelete = (studentId, id) => {
    ...
    setVacationScheduleId(id);
    setShowConfirm(true);
  };

  const handleConfirmDelete = (studentId, vacationScheduleId) => {
    const token = localStorage.getItem("token");

    if (token) {
      axios
        .delete(
          `${BASE_URL}/commander/${studentId}/vacationSchedule/${vacationScheduleId}`,
          {
            headers: {
              token: `Bearer ${token}`,
            },
        ...
        .then(() => {
          setVacationSchedules(
            vacationSchedules.filter(
              (vacationSchedule) => vacationSchedule.id !== vacationScheduleId
            )
          );
          handleNotify(
            "success",
            "Thành công!",
            "Xóa học viên đi tranh thủ thành công"
          );
          fetchVacationSchedules();
        })
  ...
  const fetchVacationSchedules = async () => {
    ...
    if (token) {
      try {
        const res = await axios.get(`${BASE_URL}/commander/vacationSchedules`, {
          headers: {
            token: `Bearer ${token}`,
          },
        });

        setVacationSchedules(res.data);
      } catch (error) {
        console.log(error);
      }
    }
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    router.push(`/admin/vacation-schedules?unit=${unit}`);
    const token = localStorage.getItem("token");

    if (token) {
      try {
        const res = await axios.get(
          `${BASE_URL}/commander/vacationSchedules?unit=${unit}`,
          {
            headers: {
              token: `Bearer ${token}`,
            },
        ...
        if (res.status === 404) setVacationSchedules([]);
          ...
          {showFormEdit ? (
            <div className="fixed text-start inset-0 mt-16 flex items-center justify-center z-30">
              ...
              >
                ...
                <form
                  onSubmit={(e) => handleUpdate(e, vacationScheduleId)}
                ...
                <div className="fixed top-0 left-0 z-20 w-full h-full bg-slate-400 bg-opacity-50 flex justify-center items-center">
                  ...
                  >
                    ...
                    <div className="flex justify-center items-center space-x-4">
                      ...
                      <button
                        onClick={() =>
                          handleConfirmDelete(studentId, vacationScheduleId)
                        }
                        type="submit"
                ...
                <div className="overflow-x-auto">
                  <table className="table-auto w-full divide-y divide-gray-200 dark:divide-gray-700 border border-gray-200 dark:border-gray-700 rounded-lg">
                    ...
                    <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                      {vacationSchedules && vacationSchedules.length > 0 ? (
                        vacationSchedules.map((item, index) => (
                          <tr
                            key={item.id}
                        ...
                        <tr>
                          ...
                          >
                            <div className="flex flex-col items-center">
                              ...
                              <p className="text-sm">
                                Không tìm thấy lịch tranh thủ nào
                              </p>
                            </div>
                          </td>
                        </tr>
...
export default VacationSchedules;
